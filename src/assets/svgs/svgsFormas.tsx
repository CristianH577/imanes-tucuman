import type { TypeObjectGeneral } from "../../consts/types";

export const SVGRedondo = (props: TypeObjectGeneral) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox={`0 0 104 48`}
    fill="none"
    stroke="currentColor"
    width={104}
    height={48}
    {...props}
  >
    <g strokeWidth={3} transform="translate(-.018 -29.969)">
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M2 44v20c10 16 86 16 100 0V44"
      />
      <ellipse cx={52} cy={44} rx={50} ry={12} />
    </g>
  </svg>
);
export const SVGCuadrado = (props: TypeObjectGeneral) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 61 49"
    fill="currentColor"
    stroke={props?.stroke || "currentColor"}
    width={61}
    height={49}
    {...props}
  >
    <path
      d="M60.393 38.074c-.003.012-.01.022-.014.034a1.016 1.016 0 0 1-.086.207c-.015.027-.037.049-.055.075-.032.045-.063.091-.102.131-.026.026-.057.047-.086.07-.032.026-.061.056-.097.079-.008.005-.018.007-.026.012s-.014.012-.023.017l-18 9.831a.999.999 0 0 1-.938.011l-40-20.644c-.014-.007-.024-.019-.037-.027-.016-.009-.035-.014-.051-.025-.03-.02-.054-.046-.081-.069-.027-.022-.055-.043-.08-.068a.942.942 0 0 1-.105-.131c-.017-.026-.037-.049-.053-.076a.993.993 0 0 1-.08-.19c-.006-.018-.015-.033-.02-.051a1.02 1.02 0 0 1-.033-.253V10.295a1 1 0 0 1 .586-.91l19-8.644c.015-.007.032-.004.047-.01a.977.977 0 0 1 .367-.08c.088 0 .175.014.26.037.028.008.053.021.081.031.041.015.084.026.123.047l39 20.441c.33.172.536.513.536.885V37.82a.978.978 0 0 1-.033.254Zm-18.981 8.445 15.841-8.652-36.851-18.113-16.656 7.325 37.666 19.44ZM2.426 25.442l17-7.873V3.204l-17 7.735v14.503Zm19-22.138v14.187l37 18.703V22.697l-37-19.393Z"
      style={{
        strokeWidth: 1,
        transformBox: "fill-box",
        transformOrigin: "50% 50%",
      }}
      transform="rotate(180 0 0)"
    />
  </svg>
);
export const SVGRedondoFresado = (props: TypeObjectGeneral) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 125 86"
    fill="none"
    stroke="currentColor"
    width={125}
    height={86}
    {...props}
  >
    <g transform="translate(.175 -45.221)">
      <ellipse cx={62.409} cy={77.107} strokeWidth={3} rx={60} ry={30} />
      <ellipse cx={62.523} cy={86.07} strokeWidth={2} rx={22.087} ry={10.545} />
      <ellipse
        cx={166.46}
        cy={172.031}
        strokeWidth={2}
        rx={46.082}
        ry={20.309}
        style={{}}
        transform="matrix(1 .00013 0 1 -103.723 -94.593)"
      />
      <path
        strokeDasharray={2}
        d="m36.083 60.474 14.14 15.051m57.769 1.215L84.61 86.103"
      />
      <path
        strokeWidth={3}
        d="M2.328 76.891 2.249 99.24m120.077-23.716-.191 23.506m-119.99-.274c-.107 37.04 114.363 43.431 120.065.158"
      />
      <path
        strokeDasharray={2}
        d="m74.537 75.525 14.148-15.051M40.436 86.103l-23.349-7.969"
      />
    </g>
  </svg>
);
export const SVGCuadradoFresado = (props: TypeObjectGeneral) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 61 49"
    fill="currentColor"
    stroke="currentColor"
    width={61}
    height={49}
    {...props}
  >
    <path
      d="M60.363 37.914c-.003.012-.01.022-.014.034a1.016 1.016 0 0 1-.086.207c-.015.027-.037.049-.055.075-.032.045-.063.091-.102.131-.026.026-.057.047-.086.07-.032.026-.061.056-.097.079-.008.005-.018.007-.026.012s-.014.012-.023.017l-18 9.831a.999.999 0 0 1-.938.011l-40-20.644c-.014-.007-.024-.019-.037-.027-.016-.009-.035-.014-.051-.025-.03-.02-.054-.046-.081-.069-.027-.022-.055-.043-.08-.068a.942.942 0 0 1-.105-.131c-.017-.026-.037-.049-.053-.076a.993.993 0 0 1-.08-.19c-.006-.018-.015-.033-.02-.051a1.02 1.02 0 0 1-.033-.253V10.135a1 1 0 0 1 .586-.91l19-8.644c.015-.007.032-.004.047-.01a.977.977 0 0 1 .367-.08c.088 0 .175.014.26.037.028.008.053.021.081.031.041.015.084.026.123.047l39 20.441c.33.172.536.513.536.885V37.66a.978.978 0 0 1-.033.254Zm-18.981 8.445 15.841-8.652-36.851-18.113-16.656 7.325 37.666 19.44ZM2.396 25.282l17-7.873V3.044l-17 7.735v14.503Zm19-22.138v14.187l37 18.703V22.537l-37-19.393Z"
      style={{
        strokeWidth: 1,
        transformBox: "fill-box",
        transformOrigin: "50% 50%",
      }}
      transform="rotate(180 0 0)"
    />
    <g fill="none" transform="matrix(.18733 .05212 0 .23653 19.766 -5.337)">
      <ellipse
        cx={58.17}
        cy={153.736}
        strokeDasharray={4}
        strokeWidth={5}
        rx={22.087}
        ry={10.545}
      />
      <ellipse
        cx={166.46}
        cy={172.031}
        strokeWidth={5}
        rx={46.082}
        ry={20.309}
        transform="translate(-103.723 -94.593)"
      />
      <path
        strokeDasharray={2}
        d="m36.083 60.474 13.385 49.342m58.524-27.087-22.064 34.939m-12.774-10.006 15.531-47.188M40.62 121.211 17.087 78.134"
      />
      <ellipse
        cx={62.416}
        cy={118.038}
        strokeDasharray={4}
        strokeWidth={5}
        rx={22.087}
        ry={10.545}
      />
      <path
        strokeDasharray={2}
        d="m54.511 127.953-4.763 36.873m35.84-46.043-5.429 36.663m-10.127-11.488 3.433-34.042m-36.346 38.991 3.689-28.714"
      />
    </g>
  </svg>
);
export const SVGArrastre = (props: TypeObjectGeneral) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox={`0 0 18 17`}
    fill="none"
    stroke="currentColor"
    width={18}
    height={17}
    {...props}
  >
    <circle
      cx={9.061}
      cy={5.676}
      r={5.164}
      fill="none"
      strokeWidth={5}
      style={{
        strokeWidth: 0.861,
      }}
    />
    <g
      fill="none"
      strokeWidth={7.553}
      transform="matrix(.03867 0 0 .05248 6.934 7.98)"
    >
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M2 44v20c10 16 86 16 100 0V44"
      />
      <ellipse cx={52} cy={44} rx={50} ry={12} />
    </g>
    <g
      fill="none"
      strokeWidth={3}
      transform="matrix(.17212 0 0 .17212 .034 3.58)"
    >
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M2 44v20c10 16 86 16 100 0V44"
      />
      <ellipse cx={52} cy={44} rx={50} ry={12} />
    </g>
  </svg>
);

//MEDIDAS
export const SVGCuadradoMeasures = (props: TypeObjectGeneral) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 103 68"
    fill={"currentColor"}
    width={103}
    height={68}
    {...props}
  >
    <path
      d="M 80.334 39.013 C 80.331 39.025 80.324 39.035 80.32 39.047 C 80.298 39.119 80.271 39.189 80.234 39.254 C 80.219 39.281 80.197 39.303 80.179 39.329 C 80.147 39.374 80.116 39.42 80.077 39.46 C 80.051 39.486 80.02 39.507 79.991 39.53 C 79.959 39.556 79.93 39.586 79.894 39.609 C 79.886 39.614 79.876 39.616 79.868 39.621 C 79.86 39.626 79.854 39.633 79.845 39.638 L 61.845 49.469 C 61.553 49.628 61.203 49.633 60.907 49.48 L 20.907 28.836 C 20.893 28.829 20.883 28.817 20.87 28.809 C 20.854 28.8 20.835 28.795 20.819 28.784 C 20.789 28.764 20.765 28.738 20.738 28.715 C 20.711 28.693 20.683 28.672 20.658 28.647 C 20.618 28.607 20.585 28.562 20.553 28.516 C 20.536 28.49 20.516 28.467 20.5 28.44 C 20.466 28.38 20.441 28.316 20.42 28.25 C 20.414 28.232 20.405 28.217 20.4 28.199 C 20.379 28.117 20.367 28.032 20.367 27.946 L 20.367 11.234 C 20.367 10.842 20.596 10.486 20.953 10.324 L 39.953 1.68 C 39.968 1.673 39.985 1.676 40 1.67 C 40.118 1.622 40.241 1.59 40.367 1.59 C 40.455 1.59 40.542 1.604 40.627 1.627 C 40.655 1.635 40.68 1.648 40.708 1.658 C 40.749 1.673 40.792 1.684 40.831 1.705 L 79.831 22.146 C 80.161 22.318 80.367 22.659 80.367 23.031 L 80.367 38.759 C 80.367 38.846 80.356 38.931 80.334 39.013 Z M 61.353 47.458 L 77.194 38.806 L 40.343 20.693 L 23.687 28.018 L 61.353 47.458 Z M 22.367 26.381 L 39.367 18.508 L 39.367 4.143 L 22.367 11.878 L 22.367 26.381 Z M 41.367 4.243 L 41.367 18.43 L 78.367 37.133 L 78.367 23.636 L 41.367 4.243 Z"
      transform="matrix(-1, 0, 0, -1, 0.000007, 0.000002)"
      style={{
        // WebkitTransformBox: "fill-box",
        // MsTransformBox: "fill-box",
        transformBox: "fill-box",
        WebkitTransformOrigin: "50% 50%",
        // MsTransformOrigin: "50% 50%",
        transformOrigin: "50% 50%",
      }}
    ></path>

    <g
      transform="matrix(2.429822, 0.428438, -0.163969, 0.929912, -92.642012, -20.15154)"
      style={{
        WebkitTransformOrigin: "122.366px 62.841px",
        // MsTransformOrigin: "122.366px 62.841px",
        transformOrigin: "122.366px 62.841px",
      }}
    >
      <path d="M 116.59 52.228 L 118.187 53.432 L 112.169 61.418 L 110.572 60.214 L 116.59 52.228 Z"></path>
      <path d="M 122.082 57.314 L 122.36 59.294 L 119.8 59.655 L 126.135 64.429 L 125.776 61.868 L 127.756 61.59 L 128.591 67.532 L 122.65 68.366 L 122.372 66.386 L 124.932 66.026 L 118.596 61.252 L 118.957 63.813 L 116.976 64.091 L 116.141 58.15 L 122.082 57.314 Z"></path>
      <path d="M 134.16 65.468 L 132.563 64.264 L 126.545 72.25 L 128.142 73.454 L 134.16 65.468 Z"></path>
    </g>

    <g
      transform="matrix(0.906309, -0.422615, 0.422623, 0.906309, -42.193092, -19.162357)"
      style={{
        WebkitTransformOrigin: "115.681px 69.971px",
        // MsTransformOrigin: "115.681px 69.971px",
        transformOrigin: "115.681px 69.971px",
      }}
    >
      <path d="M 104.681 64.971 L 106.681 64.971 L 106.681 74.971 L 104.681 74.971 L 104.681 64.971 Z"></path>
      <path d="M 112.129 65.728 L 113.543 67.143 L 111.714 68.971 L 119.647 68.971 L 117.819 67.143 L 119.233 65.728 L 123.476 69.971 L 119.233 74.214 L 117.819 72.799 L 119.647 70.971 L 111.714 70.971 L 113.543 72.799 L 112.129 74.214 L 107.886 69.971 L 112.129 65.728 Z"></path>
      <path d="M 126.681 64.971 L 124.681 64.971 L 124.681 74.971 L 126.681 74.971 L 126.681 64.971 Z"></path>
    </g>

    <g
      transform="matrix(-0.000001, 0.780502, -0.921713, 0.000002, -17.726858, -5.692921)"
      style={{
        WebkitTransformOrigin: "104.621px 37.398px",
        // MsTransformOrigin: "104.621px 37.398px",
        transformOrigin: "104.621px 37.398px",
      }}
    >
      <path d="M 93.621 32.398 L 95.621 32.398 L 95.621 42.398 L 93.621 42.398 L 93.621 32.398 Z"></path>
      <path d="M 101.069 33.155 L 102.483 34.569 L 100.654 36.398 L 108.587 36.398 L 106.759 34.569 L 108.173 33.155 L 112.416 37.398 L 108.173 41.64 L 106.759 40.226 L 108.587 38.398 L 100.655 38.398 L 102.483 40.226 L 101.069 41.64 L 96.826 37.398 L 101.069 33.155 Z"></path>
      <path d="M 115.621 32.398 L 113.621 32.398 L 113.621 42.398 L 115.621 42.398 L 115.621 32.398 Z"></path>
    </g>

    <text style={{ whiteSpace: "pre" }} x="92.313" y="35.779" fontSize="12">
      C
    </text>
    <text style={{ whiteSpace: "pre" }} x="73.486" y="65.041" fontSize="12">
      B
    </text>
    <text style={{ whiteSpace: "pre" }} x="17.723" y="53.703" fontSize="12">
      A
    </text>
  </svg>
);
export const SVGCuadradoFresadoMeasures = (props: TypeObjectGeneral) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 102 64"
    fill={"currentColor"}
    stroke="none"
    width={102}
    height={64}
    {...props}
  >
    <path
      d="M 81.007 37.423 C 81.004 37.435 80.997 37.445 80.993 37.457 C 80.971 37.529 80.944 37.599 80.907 37.664 C 80.892 37.691 80.87 37.713 80.852 37.739 C 80.82 37.784 80.789 37.83 80.75 37.87 C 80.724 37.896 80.693 37.917 80.664 37.94 C 80.632 37.966 80.603 37.996 80.567 38.019 C 80.559 38.024 80.549 38.026 80.541 38.031 C 80.533 38.036 80.527 38.043 80.518 38.048 L 62.518 47.879 C 62.226 48.038 61.876 48.043 61.58 47.89 L 21.58 27.246 C 21.566 27.239 21.556 27.227 21.543 27.219 C 21.527 27.21 21.508 27.205 21.492 27.194 C 21.462 27.174 21.438 27.148 21.411 27.125 C 21.384 27.103 21.356 27.082 21.331 27.057 C 21.291 27.017 21.258 26.972 21.226 26.926 C 21.209 26.9 21.189 26.877 21.173 26.85 C 21.139 26.79 21.114 26.726 21.093 26.66 C 21.087 26.642 21.078 26.627 21.073 26.609 C 21.052 26.527 21.04 26.442 21.04 26.356 L 21.04 9.644 C 21.04 9.252 21.269 8.896 21.626 8.734 L 40.626 0.09 C 40.641 0.083 40.658 0.086 40.673 0.08 C 40.791 0.032 40.914 0 41.04 0 C 41.128 0 41.215 0.014 41.3 0.037 C 41.328 0.045 41.353 0.058 41.381 0.068 C 41.422 0.083 41.465 0.094 41.504 0.115 L 80.504 20.556 C 80.834 20.728 81.04 21.069 81.04 21.441 L 81.04 37.169 C 81.04 37.256 81.029 37.341 81.007 37.423 Z M 62.026 45.868 L 77.867 37.216 L 41.016 19.103 L 24.36 26.428 L 62.026 45.868 Z M 23.04 24.791 L 40.04 16.918 L 40.04 2.553 L 23.04 10.288 L 23.04 24.791 Z M 42.04 2.653 L 42.04 16.84 L 79.04 35.543 L 79.04 22.046 L 42.04 2.653 Z"
      style={{
        transformBox: "fill-box",
        transformOrigin: "50% 50%",
      }}
      transform="matrix(-1, 0, 0, -1, 0, 0)"
    ></path>

    <g
      transform="matrix(2.429822, 0.428438, -0.163969, 0.929912, -91.968475, -21.741083)"
      style={{
        transformOrigin: "122.366px 62.841px",
      }}
    >
      <path d="M 116.59 52.228 L 118.187 53.432 L 112.169 61.418 L 110.572 60.214 L 116.59 52.228 Z"></path>
      <path d="M 122.082 57.314 L 122.36 59.294 L 119.8 59.655 L 126.135 64.429 L 125.776 61.868 L 127.756 61.59 L 128.591 67.532 L 122.65 68.366 L 122.372 66.386 L 124.932 66.026 L 118.596 61.252 L 118.957 63.813 L 116.976 64.091 L 116.141 58.15 L 122.082 57.314 Z"></path>
      <path d="M 134.16 65.468 L 132.563 64.264 L 126.545 72.25 L 128.142 73.454 L 134.16 65.468 Z"></path>
    </g>

    <g
      transform="matrix(0.906309, -0.422615, 0.422623, 0.906309, -41.519562, -20.751902)"
      style={{
        transformOrigin: "115.681px 69.971px",
      }}
    >
      <path d="M 104.681 64.971 L 106.681 64.971 L 106.681 74.971 L 104.681 74.971 L 104.681 64.971 Z"></path>
      <path d="M 112.129 65.728 L 113.543 67.143 L 111.714 68.971 L 119.647 68.971 L 117.819 67.143 L 119.233 65.728 L 123.476 69.971 L 119.233 74.214 L 117.819 72.799 L 119.647 70.971 L 111.714 70.971 L 113.543 72.799 L 112.129 74.214 L 107.886 69.971 L 112.129 65.728 Z"></path>
      <path d="M 126.681 64.971 L 124.681 64.971 L 124.681 74.971 L 126.681 74.971 L 126.681 64.971 Z"></path>
    </g>

    <g
      transform="matrix(0, 0.780502, -0.921713, 0, -17.053328, -7.282456)"
      style={{
        transformOrigin: "104.621px 37.398px",
      }}
    >
      <path d="M 93.621 32.398 L 95.621 32.398 L 95.621 42.398 L 93.621 42.398 L 93.621 32.398 Z"></path>
      <path d="M 101.069 33.155 L 102.483 34.569 L 100.654 36.398 L 108.587 36.398 L 106.759 34.569 L 108.173 33.155 L 112.416 37.398 L 108.173 41.64 L 106.759 40.226 L 108.587 38.398 L 100.655 38.398 L 102.483 40.226 L 101.069 41.64 L 96.826 37.398 L 101.069 33.155 Z"></path>
      <path d="M 115.621 32.398 L 113.621 32.398 L 113.621 42.398 L 115.621 42.398 L 115.621 32.398 Z"></path>
    </g>

    <text x="92.986" y="34.189" fontSize="12">
      C
    </text>
    <text x="74.159" y="63.451" fontSize="12">
      B
    </text>
    <text x="18.396" y="52.113" fontSize="12">
      A
    </text>

    <g
      fill="none"
      stroke="currentColor"
      transform="matrix(0.18733, 0.05212, 0, 0.23653, 40.410019, -5.827593)"
    >
      <ellipse
        cx="58.17"
        cy="153.736"
        strokeDasharray="4"
        strokeWidth="5"
        rx="22.087"
        ry="10.545"
      ></ellipse>

      <ellipse
        cx="166.46"
        cy="172.031"
        strokeWidth="5"
        rx="46.082"
        ry="20.309"
        transform="matrix(1 0 0 1 -103.723 -94.593)"
      ></ellipse>

      <path
        strokeDasharray="2"
        d="m36.083 60.474 13.385 49.342m58.524-27.087-22.064 34.939m-12.774-10.006 15.531-47.188M40.62 121.211 17.087 78.134"
      ></path>

      <ellipse
        cx="62.416"
        cy="118.038"
        strokeDasharray="4"
        strokeWidth="5"
        rx="22.087"
        ry="10.545"
      ></ellipse>
      <path
        strokeDasharray="2"
        d="m54.511 127.953-4.763 36.873m35.84-46.043-5.429 36.663m-10.127-11.488 3.433-34.042m-36.346 38.991 3.689-28.714"
      ></path>
    </g>

    <text x="35.399" y="14.646" fontSize="10">
      D
    </text>
    <text x="250" y="86" fontSize="9" transform="translate(-210.924 -52.244)">
      d
    </text>
  </svg>
);

export const SVGRedondoMeasures = (props: TypeObjectGeneral) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 140 90"
    fill={"currentColor"}
    stroke={"currentColor"}
    width={140}
    height={90}
    {...props}
  >
    <text
      style={{ whiteSpace: "pre" }}
      x="43.626"
      y="23.624"
      fontSize="30"
      stroke="none"
    >
      A
    </text>
    <path
      stroke="none"
      d="M5.748 27.56c-1.323-.473-1.323-1.243 0-1.719l13.509-4.856c1.837-.66 4.984-.357 5.654.547.317.42-.02.866-.872 1.172l-7.749 2.782h71.506l-7.748-2.782c-1.844-.66-1-1.789 1.512-2.032 1.17-.112 2.414.007 3.271.313l13.509 4.856c1.324.476 1.324 1.246 0 1.719l-13.509 4.856c-1.844.663-4.984.359-5.661-.544-.31-.42.021-.867.878-1.175l7.748-2.783H16.29l7.749 2.783c1.837.663 1 1.792-1.52 2.034-1.161.112-2.41-.007-3.262-.315L5.748 27.56z"
    ></path>
    <text
      style={{ lineHeight: 49.4665, whiteSpace: "pre" }}
      x="270.463"
      y="450.562"
      fontSize="30.917"
      transform="matrix(1 0 0 .97035 -150.753 -362.593)"
      stroke="none"
    >
      B
    </text>
    <path
      stroke="none"
      d="M 93.948 65.179 C 93.33 64.706 93.33 63.936 93.948 63.46 L 100.291 58.604 C 101.152 57.944 102.631 58.247 102.942 59.151 C 103.092 59.571 102.935 60.017 102.535 60.323 L 98.896 63.105 L 132.462 63.105 L 128.823 60.323 C 127.96 59.663 128.356 58.534 129.534 58.291 C 130.087 58.179 130.667 58.298 131.07 58.604 L 137.413 63.46 C 138.031 63.936 138.031 64.706 137.413 65.179 L 131.07 70.035 C 130.206 70.698 128.73 70.394 128.414 69.491 C 128.267 69.071 128.423 68.624 128.823 68.316 L 132.462 65.533 L 98.896 65.533 L 102.535 68.316 C 103.397 68.979 103.004 70.108 101.819 70.35 C 101.274 70.462 100.69 70.343 100.291 70.035 L 93.948 65.179 Z"
      transform="matrix(0, 0.999996, -1.000004, 0, 0.000004, -0.000006)"
      style={{
        // WebkitTransformBox: "fill-box",
        // MsTransformBox: "fill-box",
        transformBox: "fill-box",
        WebkitTransformOrigin: "50% 50%",
        // MsTransformOrigin: "50% 50%",
        transformOrigin: "50% 50%",
      }}
    ></path>

    <g fill="none" strokeWidth="3" transform="translate(0 10.218)">
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M2 44v20c10 16 86 16 100 0V44"
      ></path>
      <ellipse cx="52" cy="44" rx="50" ry="12"></ellipse>
    </g>
  </svg>
);
export const SVGRedondoFresadoMeasures = (props: TypeObjectGeneral) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 170 200"
    fill={"currentColor"}
    stroke={"currentColor"}
    width={200}
    height={200}
    {...props}
  >
    <g>
      <text
        style={{ whiteSpace: "pre" }}
        x="158.837"
        y="94.731"
        fontSize="30"
        transform="translate(-101.914 -69.39)"
      >
        A
      </text>
      <path
        d="M108.613 100.613c-1.681-.473-1.681-1.243 0-1.719l17.179-4.856c2.337-.66 6.338-.357 7.189.547.404.42-.024.866-1.108 1.172l-9.854 2.782h90.927l-9.851-2.782c-2.346-.66-1.27-1.789 1.923-2.032 1.488-.112 3.069.007 4.159.313l17.178 4.856c1.683.476 1.683 1.246 0 1.719l-17.178 4.856c-2.345.663-6.339.359-7.199-.544-.395-.42.028-.867 1.117-1.175l9.851-2.783h-90.927l9.854 2.783c2.335.663 1.271 1.792-1.932 2.034-1.479.112-3.067-.007-4.149-.315l-17.179-4.856z"
        transform="translate(-101.914 -69.39)"
      ></path>
    </g>
    <g>
      <text
        transform="translate(28.402 55.171) matrix(1 0 0 .97035 -150.753 -362.593)"
        style={{ lineHeight: 1529.36, whiteSpace: "pre" }}
        x="270.463"
        y="450.562"
        fontSize="30.917"
      >
        B
      </text>
      <path
        d="M93.948 65.179c-.618-.473-.618-1.243 0-1.719l6.343-4.856c.861-.66 2.34-.357 2.651.547.15.42-.007.866-.407 1.172l-3.639 2.782h33.566l-3.639-2.782c-.863-.66-.467-1.789.711-2.032.553-.112 1.133.007 1.536.313l6.343 4.856c.618.476.618 1.246 0 1.719l-6.343 4.856c-.864.663-2.34.359-2.656-.544-.147-.42.009-.867.409-1.175l3.639-2.783H98.896l3.639 2.783c.862.663.469 1.792-.716 2.034-.545.112-1.129-.007-1.528-.315l-6.343-4.856z"
        transform="translate(28.402 55.171) rotate(90 0 0)"
        style={{
          // WebkitTransformBox: "fill-box",
          // MsTransformBox: "fill-box",
          transformBox: "fill-box",
          WebkitTransformOrigin: "50% 50%",
          // MsTransformOrigin: "50% 50%",
          transformOrigin: "50% 50%",
        }}
      ></path>
    </g>
    <g>
      <text
        transform="translate(-4.816 37.935) matrix(1 0 0 .97035 -209.189 -414.102)"
        style={{ lineHeight: 1529.36, whiteSpace: "pre" }}
        x="270.463"
        y="450.562"
        fontSize="30.917"
      >
        D
      </text>
      <path
        d="M25.717 28.011c-1.27-.265-1.27-.696 0-.962l13.03-2.718c1.768-.37 4.806-.2 5.445.306.309.235-.015.485-.836.656L35.88 26.85h68.951l-7.474-1.557c-1.773-.369-.96-1.001 1.46-1.137 1.135-.063 2.328.004 3.155.175l13.03 2.718c1.269.266 1.269.697 0 .962l-13.03 2.718c-1.774.371-4.806.201-5.456-.304-.302-.235.018-.485.841-.658l7.474-1.558H35.88l7.476 1.558c1.77.371.964 1.003-1.471 1.139-1.12.062-2.319-.004-3.138-.177l-13.03-2.718z"
        style={{
          // WebkitTransformBox: "fill-box",
          // MsTransformBox: "fill-box",
          transformBox: "fill-box",
          WebkitTransformOrigin: "50% 50%",
          // MsTransformOrigin: "50% 50%",
          transformOrigin: "50% 50%",
        }}
        transform="translate(-4.816 37.935)"
      ></path>
    </g>
    <g>
      <text
        transform="translate(-64.373 173.792) matrix(1 0 0 .97035 -150.097 -417.048)"
        style={{ lineHeight: 1529.36, whiteSpace: "pre" }}
        x="270.463"
        y="450.562"
        fontSize="30.917"
      >
        d
      </text>
      <path
        d="M108.973-7.443c-.61-.265-.61-.696 0-.962l6.263-2.718c.85-.37 2.31-.2 2.618.306.147.235-.009.485-.403.656l-3.592 1.557H147l-3.593-1.557c-.852-.369-.46-1.001.702-1.137.546-.063 1.119.004 1.516.175l6.265 2.718c.608.266.608.697 0 .962l-6.265 2.718c-.852.371-2.309.201-2.621-.304-.145-.235.007-.485.403-.658L147-7.245h-33.141l3.592 1.558c.851.371.465 1.003-.706 1.139-.54.062-1.115-.004-1.509-.177l-6.263-2.718z"
        style={{
          // WebkitTransformBox: "fill-box",
          // MsTransformBox: "fill-box",
          transformBox: "fill-box",
          WebkitTransformOrigin: "50% 50%",
          // MsTransformOrigin: "50% 50%",
          transformOrigin: "50% 50%",
        }}
        transform="translate(-64.373 173.792)"
      ></path>
    </g>
    <g fill="none" transform="translate(3.293 26.876)">
      <ellipse
        cx="62.409"
        cy="77.107"
        strokeWidth="3"
        rx="60"
        ry="30"
      ></ellipse>
      <ellipse
        cx="62.523"
        cy="86.07"
        strokeWidth="2"
        rx="22.087"
        ry="10.545"
      ></ellipse>
      <ellipse
        cx="166.46"
        cy="172.031"
        strokeWidth="2"
        rx="46.082"
        ry="20.309"
        transform="matrix(1 .00013 0 1 -103.723 -94.593)"
      ></ellipse>
      <path
        strokeDasharray="2"
        d="M36.083 60.474l14.14 15.051m57.769 1.215L84.61 86.103"
      ></path>
      <path
        strokeWidth="3"
        d="M2.328 76.891L2.249 99.24m120.077-23.716l-.191 23.506m-119.99-.274c-.107 37.04 114.363 43.431 120.065.158"
      ></path>
      <path
        strokeDasharray="2"
        d="M74.537 75.525l14.148-15.051M40.436 86.103l-23.349-7.969"
      ></path>
    </g>
  </svg>
);

export const SVGDragMagnetMeasures = (props: TypeObjectGeneral) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 182 138"
    fill={"currentColor"}
    stroke={"currentColor"}
    width={182}
    height={138}
    {...props}
  >
    <circle cx="91.155" cy="33.525" r="30" fill="none" strokeWidth="5"></circle>
    <g
      fill="none"
      strokeWidth="7.553"
      transform="matrix(.22466 0 0 .3049 78.8 46.909)"
    >
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M2 44v20c10 16 86 16 100 0V44"
      ></path>
      <ellipse cx="52" cy="44" rx="50" ry="12"></ellipse>
    </g>
    <g fill="none" strokeWidth="3" transform="translate(38.708 21.344)">
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M2 44v20c10 16 86 16 100 0V44"
      ></path>
      <ellipse cx="52" cy="44" rx="50" ry="12"></ellipse>
    </g>

    <g>
      <text
        x="96.699"
        y="199.14"
        fontSize="30"
        transform="matrix(1.05734 0 0 1 -22.723 -62.696)"
      >
        A
      </text>
      <path d="M41.757 110.148c-1.4-.473-1.4-1.243 0-1.72l14.283-4.855c1.943-.66 5.27-.357 5.979.547.335.42-.022.866-.922 1.172l-8.194 2.782h75.606l-8.192-2.782c-1.95-.66-1.057-1.79 1.599-2.032 1.237-.112 2.552.007 3.458.313l14.284 4.856c1.4.476 1.4 1.246 0 1.719l-14.284 4.856c-1.95.663-5.27.359-5.985-.544-.328-.42.022-.867.928-1.175l8.192-2.783H52.903l8.194 2.783c1.942.663 1.057 1.792-1.608 2.034-1.227.112-2.548-.007-3.45-.315z"></path>
    </g>

    <g>
      <text
        x="96.699"
        y="199.14"
        fontSize="30"
        transform="translate(59 -115) matrix(1.05734 0 0 1 .065 1.65)"
      >
        B
      </text>
      <path
        d="M190.648-81.254c-.47.418-1.24.424-1.719.015l-4.892-4.183c-.664-.57-.37-1.555.532-1.773.42-.102.866-.001 1.175.263l2.802 2.4-.19-22.359-2.76 2.446c-.656.583-1.787.328-2.037-.456-.115-.364.001-.754.305-1.024l4.82-4.266c.472-.418 1.242-.425 1.719-.015l4.891 4.184c.668.57.372 1.555-.529 1.774-.419.1-.867 0-1.177-.265l-2.803-2.398.19 22.358 2.761-2.446c.659-.581 1.79-.329 2.038.457.115.362 0 .754-.306 1.023l-4.82 4.265z"
        style={{
          transformBox: "fill-box",
          transformOrigin: "50% 50%",
        }}
        transform="translate(-38 170)"
      />
    </g>

    <g>
      <text
        x="96.699"
        y="199.14"
        fontSize="30"
        transform="matrix(1.05734 0 0 1 -102.193 -142.228)"
      >
        C
      </text>

      <path
        d="M139.316 135.55c-.485 1.346-1.255 1.34-1.72-.015l-4.738-13.94c-.644-1.896-.314-5.133.596-5.815.423-.317.866.027 1.164.92l2.715 7.983.62-73.537-2.849 7.945c-.676 1.891-1.797 1.014-2.018-1.572-.102-1.204.027-2.482.34-3.362l4.974-13.852c.487-1.358 1.257-1.352 1.719.014l4.738 13.935c.647 1.901.316 5.129-.593 5.817-.422.315-.866-.028-1.167-.913l-2.716-7.991-.62 73.536 2.85-7.936c.68-1.894 1.8-1.015 2.02 1.567.103 1.201-.027 2.48-.342 3.357l-4.973 13.859z"
        transform="translate(-110 -38)"
      />
    </g>
  </svg>
);
// export const SVGEsfera = ({ fill, size, height, width, ...props }) => (
//   <svg
//     xmlns="http://www.w3.org/2000/svg"
//     viewBox="0 0 140 100"
//     fill={fill || "currentColor"}
//     width={size * 140 || width || 140}
//     height={size * 100 || height || 100}
//     {...props}
//   >
//     <g transform="matrix(1, 0, 0, 1, -2.246479, -11.360136)">
//       <text
//         style={{ lineHeight: 49.4665, whiteSpace: "pre" }}
//         x="270"
//         y="458"
//         fontSize="30.917"
//         transform="translate(-2.246 -11.36) matrix(1 0 0 .97035 -150.753 -362.593)"
//       >
//         D
//       </text>
//       <path
//         d="M 68.803 60.095 C 67.471 59.622 67.471 58.852 68.803 58.376 L 82.485 53.52 C 84.342 52.86 87.532 53.163 88.204 54.067 C 88.527 54.487 88.188 54.933 87.326 55.239 L 79.476 58.021 L 151.878 58.021 L 144.029 55.239 C 142.167 54.579 143.021 53.45 145.563 53.207 C 146.755 53.095 148.006 53.214 148.876 53.52 L 162.558 58.376 C 163.89 58.852 163.89 59.622 162.558 60.095 L 148.876 64.951 C 147.013 65.614 143.829 65.31 143.147 64.407 C 142.83 63.987 143.166 63.54 144.029 63.232 L 151.878 60.449 L 79.476 60.449 L 87.326 63.232 C 89.185 63.895 88.338 65.024 85.782 65.266 C 84.606 65.378 83.345 65.259 82.485 64.951 L 68.803 60.095 Z"
//         transform="matrix(0, 0.999996, -1.000004, 0, 0.000002, 0.000002)"
//         style={{
//           WebkitTransformBox: "fill-box",
//           MsTransformBox: "fill-box",
//           transformBox: "fill-box",
//           WebkitTransformOrigin: "50% 50%",
//           MsTransformOrigin: "50% 50%",
//           transformOrigin: "50% 50%",
//         }}
//         // transform="translate(-2.246 -11.36)"
//       ></path>
//     </g>
//     <path d="M50.038 0c-38.462 0-62.5 41.666-43.27 75C26 108.333 74.076 108.333 93.306 75A50.025 50.025 0 00100 50C99.972 22.398 77.62.03 50.038 0zm42.276 50c0 2.884-8.168 7.802-23.43 10.178.246-3.366.37-6.77.37-10.178 0-15.5-2.57-31.116-7.562-40.674C79.806 14.554 92.288 31.134 92.314 50zM50.038 7.692c3.916 0 11.53 14.932 11.53 42.308 0 3.962-.164 7.654-.448 11.092-3.434.282-7.124.446-11.082.446-27.354 0-42.276-7.62-42.276-11.538C7.788 26.646 26.7 7.718 50.038 7.692zM9.394 61.664c9.552 4.994 25.156 7.566 40.644 7.566 3.393 0 6.786-.123 10.17-.37-2.374 15.274-7.288 23.448-10.17 23.448-18.852-.026-35.42-12.516-40.644-30.644zm52.298 29.008c3.022-5.768 5.156-13.782 6.37-22.634 8.846-1.216 16.816-3.366 22.618-6.374a42.477 42.477 0 01-28.988 29.008z"></path>
//   </svg>
// );
