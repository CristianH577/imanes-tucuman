...
const MotionLinkRouter = m.create(Link);

const navItemsFiltered = NAV_ITEMS.filter((nav) =>
  ["imanes", "uya", "faqs"].includes(nav.id)
);
  ...
  return (
    <nav className="sticky top-0 left-0 z-50 w-full backdrop-blur-md">
      <div className="xs:p-2 flex items-center xs:gap-4 justify-between md:justify-around w-full max-w-[1200px] h-16 place-self-center sm:p-4">
        ...
        >
          {navItemsFiltered.map((li) => (
            <li key={li.id} className="hover:scale-105 transition-transform">
              <MotionLinkRouter
                variants={{
                  hidden: { opacity: 0 },
                  visible: { opacity: 1 },
        ...
        <section className="flex items-center gap-1">
          ...
          >
            <ContentPasteSearchOutlinedIcon
              className="text-custom2 dark:text-custom1 h-6 w-fit"
