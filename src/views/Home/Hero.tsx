...
const icons = [
  {
    id: "tl",
    comp: SVGMagnet2,
    class: "top-0 left-6 sm:left-12 -rotate-[100deg]",
  },
  {
    id: "br",
    comp: SVGMagnet,
    class: "bottom-0 right-0 sm:right-10 rotate-[80deg]",
  },
  {
    id: "tr",
    comp: ElectricBoltIcon,
    class: "top-0 sm:top-8 right-0 sm:right-16 rotate-[10deg]",
  },
  {
    id: "bl",
    comp: SVGBarsStaggered,
    class: "bottom-0 left-2 sm:left-16",
  },
];
...
export default function Hero() {
  return (
    <m.section
      ...
      className="w-screen relative right-2 sm:right-7 lg:right-11 overflow-hidden shadow-md shadow-black/50 min-[2400px]:h-[1200px] pt-2 bg-radial from-custom2-10 to-custom2"
      style={{
    ...
    >
      <div className="relative flex flex-col gap-4 items-center w-full h-full max-w-7xl place-self-center">
        ...
        <article className="relative w-full flex flex-grow items-center justify-center">
          {icons.map((icon) => (
            <span key={icon.id}>
              <icon.comp className={`${icon.class} ${class_icons_main}`} />
            </span>
          ))}
