...
const ModalFullImg = lazy(() => import("./ModalFullImg"));
...
export default function ImagesSection({
  ...
  return (
    ...
    >
      ...
      <m.article
        className="relative flex items-center rounded-md shadow-md flex-1 bg-content1 border dark:border-neutral-500"
        variants={{
      ...
      >
        ...
        <Swiper
          className="w-full h-[340px] select-none content-center"
          slidesPerView={1}
          spaceBetween={100}
        ...
        >
          ...
          {imgsData.imgs.map((img, i) => (
            <SwiperSlide key={"img-" + i} className="relative content-center">
              <ImageCustom
                src={img}
                alt="Imagen seleccionada"
                className="object-contain drop-shadow-md drop-shadow-black w-full h-full max-h-[320px] max-w-[320px] xs:rounded-md"
                classes={{ wrapper: "place-self-center" }}
                width={320}
              />

              <span className="absolute right-2 bottom-1 bg-neutral-500/30 px-2 rounded-md font-bold text-second">
                {i + 1} / {total_imgs}
              </span>
            </SwiperSlide>
          ))}
