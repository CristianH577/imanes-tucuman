...
export default function CartView() {
  ...
  const handleSend = () => {
    ...
    items.forEach((item) => {
      ...
      const priceData = handlePriceData(item.priceData, qtt);
    ...
    const url = `${LINKS_SITES.whatsapp}?text=${encoded_message}`;
  ...
  return (
    <>
      <section className="max-xs:px-2">
        ...
        {downloading && (
          <span className="bg-black/50 absolute inset-0 z-50 flex items-center justify-center">
      ...
      <section className="flex gap-2 max-xs:px-2 flex-wrap justify-center">
        {buttonConsole.map((button) => (
          <Tooltip key={button.id} title={button.title} arrow>
            <span>
              <Button
                // @ts-ignore
                color={button.color}
      ...
      <section className="text-center">
        ...
        <b className="font-size-secondary text-neutral-500 dark:text-neutral-300 sm:hidden">
          <br />
